...
parser.add_argument(
    "fastq",
    nargs="+",
    help="FASTQ input. Format: fastq1 [fastq2], or comma-separated lists for each if multiple FASTQs/mate.",
)
...
parser.add_argument(
    "--outSAMunmapped", default="Within", help="Keep unmapped reads in output BAM"
)
parser.add_argument(
    "--outSAMattrRGline",
    default=["ID:rg1", "SM:sm1"],
    nargs="+",
...
parser.add_argument(
    "--varVCFfile",
    default=None,
    help="VCF for the input sample; currently supports SNPs only",
)
...
parser.add_argument(
    "--chimOutType", default=["Junctions", "WithinBAM", "SoftClip"], nargs="+", help=""
)
parser.add_argument("--chimMainSegmentMultNmax", default="1", help="")
parser.add_argument(
    "--chimOutJunctionFormat", default="0", help="Formatting for Chimeric.out.junction"
)
...
if (
    args.annotation_gtf is not None
...
cmd += (
    ...
    + args.outFilterMultimapNmax
    + " --alignSJoverhangMin "
    + args.alignSJoverhangMin
    + " --alignSJDBoverhangMin "
    + args.alignSJDBoverhangMin
    + " --outFilterMismatchNmax "
    + args.outFilterMismatchNmax
    + " --outFilterMismatchNoverLmax "
    + args.outFilterMismatchNoverLmax
    + " --alignIntronMin "
    + args.alignIntronMin
    + " --alignIntronMax "
    + args.alignIntronMax
    + " --alignMatesGapMax "
    + args.alignMatesGapMax
    + " --outFilterType "
    + args.outFilterType
    + " --outFilterScoreMinOverLread "
    + args.outFilterScoreMinOverLread
    + " --outFilterMatchNminOverLread "
    + args.outFilterMatchNminOverLread
    + " --limitSjdbInsertNsj "
    + args.limitSjdbInsertNsj
    + " --readFilesIn "
    + " ".join(args.fastq)
)
...
cmd += (
    ...
    + "."
    + " --outSAMstrandField "
    + args.outSAMstrandField
    + " --outFilterIntronMotifs "
    + args.outFilterIntronMotifs
    + " --alignSoftClipAtReferenceEnds "
    + args.alignSoftClipAtReferenceEnds
    + " --quantMode "
    + " ".join(args.quantMode)
    + " --outSAMtype "
    + " ".join(args.outSAMtype)
    + " --outSAMunmapped "
    + args.outSAMunmapped
    + " --genomeLoad "
    + args.genomeLoad
...
if int(args.chimSegmentMin) > 0:
    cmd += (
        ...
        + args.chimSegmentMin
        + " --chimJunctionOverhangMin "
        + args.chimJunctionOverhangMin
        + " --chimOutType "
        + " ".join(args.chimOutType)
        + " --chimMainSegmentMultNmax "
        + args.chimMainSegmentMultNmax
        + " --chimOutJunctionFormat {}".format(args.chimOutJunctionFormat)
    )
cmd += (
    " --outSAMattributes "
    + " ".join(args.outSAMattributes)
    + " --outSAMattrRGline "
    + " ".join(args.outSAMattrRGline)
)
...
with cd(args.output_dir):
    ...
    print(
        "[" + datetime.now().strftime("%b %d %H:%M:%S") + "] Finished sorting BAM",
        flush=True,
    )

    # index BAM
    print(
        "[" + datetime.now().strftime("%b %d %H:%M:%S") + "] Indexing BAM", flush=True
    )
    cmd = "samtools index " + args.prefix + ".Aligned.sortedByCoord.out.bam"
    subprocess.check_call(cmd, shell=True, executable="/bin/bash")
    print(
        "[" + datetime.now().strftime("%b %d %H:%M:%S") + "] Finished indexing BAM",
        flush=True,
    )

    # rename and compress outputs
    subprocess.check_call(
        "gzip " + args.prefix + ".SJ.out.tab", shell=True, executable="/bin/bash"
    )
    with cd(args.prefix + "._STARpass1"):
        os.rename("SJ.out.tab", args.prefix + ".SJ.pass1.out.tab")
        subprocess.check_call(
            "gzip " + args.prefix + ".SJ.pass1.out.tab",
            shell=True,
            executable="/bin/bash",
        )
    ...
    if os.path.exists(args.prefix + ".ReadsPerGene.out.tab"):
        subprocess.check_call(
            "gzip " + args.prefix + ".ReadsPerGene.out.tab",
            shell=True,
            executable="/bin/bash",
        )
    ...
    if os.path.exists(args.prefix + ".Chimeric.out.junction"):
        subprocess.check_call(
            "gzip " + args.prefix + ".Chimeric.out.junction",
            shell=True,
            executable="/bin/bash",
        )
