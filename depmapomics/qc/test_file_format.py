...
PARAMS_test_symbol_and_entrezid_in_column = [
    x["file"] for x in FILE_ATTRIBUTES if x["ismatrix"] and x["gene_id"] == "entrez"
]


@pytest.mark.parametrize(
    "data", PARAMS_test_symbol_and_entrezid_in_column, indirect=True
)
@pytest.mark.format
...
def test_duplicate_rows(data):
    assert not data.duplicated().any(), "some rows are duplicates"


PARAMS_test_symbol_and_enstid_in_column = [
    x["file"] for x in FILE_ATTRIBUTES if x["ismatrix"] and x["gene_id"] == "enst"
]
...
def test_symbol_and_enstid_in_column(data):
    ...
    assert (
        matches1.notnull().all() & matches2.notnull().any()
...
PARAMS_test_symbol_and_ensgid_in_column = [
    x["file"] for x in FILE_ATTRIBUTES if x["ismatrix"] and x["gene_id"] == "ensg"
]
...
PARAMS_test_null_values = [
    pytest.param(x["file"], marks=pytest.mark.xfail(strict=True))
    if x["hasNA"]
    else x["file"]
...
def test_expression_logtransform(data):
    ...
    assert (
        not data.sum(axis=1).map(lambda x: np.isclose(x, 1e6, rtol=1e-4)).all()
