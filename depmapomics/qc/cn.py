...
def plotCNchanges(
    ...
    """
    grouped = pd.concat([prevgenecn.stack(), newgenecn.stack()], axis=1)
    grouped.columns = [prevname, newname]

    grouped.reset_index(inplace=True)
    grouped.rename(columns={"level_0": depmap_id, "level_1": "gene"}, inplace=True)
    sources = pd.merge(
        prevsegments[[depmap_id, source]].drop_duplicates(),
        newsegments[[depmap_id, source]].drop_duplicates(),
        on=depmap_id,
        suffixes=["_" + prevname, "_" + newname],
    )

    sources["source_change"] = sources.apply(
        lambda x: "{:s} -> {:s}".format(
            x[source + "_" + prevname], x[source + "_" + newname]
        ),
        axis=1,
    )
    ...
    sns.scatterplot(
        data=grouped.sample(1000000, random_state=0),
        x=prevname,
        y=newname,
        hue="source_change",
