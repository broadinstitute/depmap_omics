...
def plot_rnaseqc_results(workspace, samplelist, output_path='data/rna_qcs/', 
  ...
  assert pd.Series(rnaqc).map(lambda x: x[0]).notnull().all(), "Some samples have no QC data"
  qcs = pd.DataFrame()
