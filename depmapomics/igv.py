...
def get_gcloud_auth_token():
    parrun(
        [
            "echo $(gcloud auth application-default print-access-token) > /tmp/gcloud_token"
        ],
        cores=1,
    )
...
def load_sample_tracker():
    ...
    depmap_samples.loc[
        depmap_samples["datatype"].isin(["wes", "rna", "wgs"]), "hg19_bam"
    ] = depmap_samples["legacy_bam_filepath"]
    depmap_samples.loc[
        depmap_samples["datatype"].isin(["wes", "rna", "wgs"]), "hg19_bai"
    ] = depmap_samples["legacy_bai_filepath"]
    depmap_samples.loc[
        depmap_samples["datatype"].isin(["RRBS", "raindance", "hybrid_capture"]),
        "hg19_bam",
    ] = depmap_samples["internal_bam_filepath"]
    depmap_samples.loc[
        depmap_samples["datatype"].isin(["RRBS", "raindance", "hybrid_capture"]),
        "hg19_bai",
    ] = depmap_samples["internal_bai_filepath"]

    depmap_samples.loc[
        depmap_samples["datatype"].isin(["wes", "rna", "wgs"]), "hg38_bam"
    ] = depmap_samples["internal_bam_filepath"]
    depmap_samples.loc[
        depmap_samples["datatype"].isin(["wes", "rna", "wgs"]), "hg38_bai"
    ] = depmap_samples["internal_bai_filepath"]
...
def subset_depmap_samples(depmap_samples, arxspan_ids, genome, datatypes=None):
    depmap_samples_subset = depmap_samples[
        depmap_samples["arxspan_id"].isin(arxspan_ids)
    ]
    depmap_samples_subset = depmap_samples_subset.dropna(
        subset=["{}_bam".format(genome), "{}_bai".format(genome)]
    )
    if datatypes is not None:
        depmap_samples_subset = depmap_samples_subset[
            depmap_samples_subset["datatype"].isin(datatypes)
        ]
...
class depmap_igv(igv.Browser):
    ...
    def get_the_track(self, row, fileformat="bam"):
        ...
        self.load_track(
            {
                "name": "{} ({}): {}".format(
                    sample_info["arxspan_id"],
                    sample_info["stripped_cell_line_name"],
                    sample_info["datatype"],
                ),
                "url": sample_info[
                    "{}_bam".format(self.config["genome"])
                ],  # +self.payer_project_uri_parameter,
                "indexURL": sample_info[
                    "{}_bai".format(self.config["genome"])
                ],  # +self.payer_project_uri_parameter,
                "format": fileformat,
            }
        )
    ...
    def get_the_tracks(self, arxspan_ids, datatypes=None):
        ...
        """
        self.depmap_samples_subset = subset_depmap_samples(
            self.depmap_samples, arxspan_ids, self.config["genome"], datatypes=datatypes
        )
