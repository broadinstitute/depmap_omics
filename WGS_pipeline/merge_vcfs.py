...
def solve_phasing(mut_s, geno):
    ...
    for val in pattern:
        if (val[1], val[0]) in pattern and val[1] != val[0]:
            # TODO: if RNA: RNA pick RNA version
            # TODO: check that we don't have cases where the next row has a phasing coming from another profile
        ...
        for val in pattern:
            if (val[0] in geno and val[1] in geno) and not (
                ...
                if val[0] in rep or val[1] in rep:
                    print("Cannot handle this for now:\n" + str(site[:5] + site[9:]))
                    # conflicts = True
                    break
...
RNAseq mutation file else "." or "nREF" for non ref">\n'
                    ...
                    # quick:
                    if len(set(mut_s[0])) == 1:
                        geno = mut_s[0][0]
                        allele_specific = "."
                    else:
                        geno, conflicts = solve_phasing(mut_s, to_add[0])
