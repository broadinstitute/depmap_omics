...
#docker run -v $(pwd):$(pwd) us.gcr.io/cds-docker-containers/vcf2maf:test perl vcf2maf/vcf2maf.pl --input-vcf $(pwd)/test.vcf --output-maf $(pwd)/test.maf --ref $(pwd)/Homo_sapiens_assembly38.fasta.gz --vep-path /opt/conda/envs/vep/bin/ --vep-data $(pwd)/vep_data/ --ncbi-build GRCh38 --tmp-dir $(pwd)/
