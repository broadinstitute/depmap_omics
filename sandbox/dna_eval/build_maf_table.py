...
def parse_arguments():
    ...
    parser.add_argument("--ids", default='', type=str, required=False, help="CDS IDs to constraint the data query")
...
def main():
    ...
    if args.dumpschema:
        ...
        #)
        #concatenate_tables(f"{args.dest}.merged_maf_latest", client, transfers, parallelism=14)
        #concatenate_tables(f"{args.dest}.merged_maf_latest2", client, transfers, parallelism=14)
        #concatenate_tables(f"{args.dest}.merged_maf_latest3", client, transfers, parallelism=14)
        #concatenate_tables(f"{args.dest}.merged_maf_latest5", client, transfers, parallelism=14)
        #concatenate_tables(f"{args.dest}.merged_maf_latest7", client, transfers, parallelism=14)
        concatenate_tables(f"{args.dest}.merged_maf_latest8", client, transfers, parallelism=14)
