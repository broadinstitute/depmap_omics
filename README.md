...
`mutations.py` contains `postProcess()`, the function responsible for postprocessing aggregated MAF files, genotyped mutation matrices (hot spot, driver, and damaging), binary guide mutation matrices, and structural variants (SVs). The mutation post processing function does the following annotating/filtering steps:

* Annotate likely immortalized mutations (occurence > 0.05 and is not a hot spot)
