...
4. **CNV_sample_XX** [gatk/CNV_Somatic_Pair_WorkflowSnapshot ID: 9](https://portal.firecloud.org/?return=terra#methods/gatk/CNV_Somatic_Pair_Workflow/9/wdl): This pipeline recieves read counts, which are the output of previous step (*gatk CollectReadCounts*), and determines the copy number segments and assigns them the deletion/amplification/neutral status. The following tools are run in sequence:
