...
readOneFile = function(infile, exons, minCov){
  ...
  #infile= 'fh_A253_SALIVARY_GLAND-Tumor.J.txt' infile = '~/Downloads/fh_22RV1_PROSTATE-Tumor.J2.txt'
  if(FALSE){
    x = read.table(infile, header=TRUE)  # firehose version 
    
  }else{
    x = read.table(infile, header=FALSE, stringsAsFactors = FALSE)
    x = x[,-(4:6)]
...
juncReadFN = as.character(args[[1]])  #'~/Downloads/22RV1_PROSTATE.SJ.out.tab' #'fh_A253_SALIVARY_GLAND-Tumor.J.txt'
...
#args = c('~/Downloads/22RV1_PROSTATE.SJ.out.tab','/xchip/cle/Resources/ExonUsage/exons_gencodev19.RData','~/Desktop/output.txt','~/Desktop/output.RData', '10')
