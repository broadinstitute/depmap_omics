...
##############################
    ...
    call MAFPonFilter as agilentMAFPonFilter{
        input:
            stub="agilent_pon",
    ...
    call MAFPonFilter as iceMAFPonFilter{
        input:
            stub="ice_pon",
            MAFFile=Oncotate_Task.WXS_Mutation_M1_SNV_M2_INDEL_Strelka_INDEL_annotated_maf,
            pairName=pairName,
    ...
    call merge_mafs_task {
        input:
            oxoGOBF_maf=oxoGOBF.WXS_Mutation_OBF_filtered_maf,
            ffpeOBF_maf=ffpeOBF.WXS_Mutation_OBF_filtered_maf,
            agilent_pon_maf=agilentMAFPonFilter.passMaf,
            ice_pon_maf=iceMAFPonFilter.passMaf,
            blat_maf=blat.blat_results,            
            pairName=pairName,
    ...
    output {
        ...
        File MUTECT1_CW=Gather_Task.MUTECT1_CW
        ...
        # MAFPoNFilter - Agilent PoN
        File agilent_pon_filter_passed_mutations=agilentMAFPonFilter.passMaf
        File agilent_pon_filter_passed_and_rejected_mutations=agilentMAFPonFilter.allMaf
        Int agilent_pon_filter_number_mutations_passed=agilentMAFPonFilter.num_passed_mutations
        Int agilent_pon_filter_number_mutations_rejected=agilentMAFPonFilter.num_rejected_mutations      
        # MAFPoNFilter - ICE PoN
        File ice_pon_filter_passed_mutations=iceMAFPonFilter.passMaf
        File ice_pon_filter_passed_and_rejected_mutations=iceMAFPonFilter.allMaf
        Int ice_pon_filter_number_mutations_passed=iceMAFPonFilter.num_passed_mutations
        Int ice_pon_filter_number_mutations_rejected=iceMAFPonFilter.num_rejected_mutations
        # Blat Re-Aligner
        ...
        ####### Absolute #######
        File absolute_plot=absolute.absolute_plot
...
CODE
    ...
    runtime {
        docker: "gcr.io/broad-cga-lelagina-wuclonal/cga_production_pipeline_wes:v2.1.7.cl"
...
task PicardMultipleMetrics_Task {
    ...
    command <<<
        ...
        echo Calling CollectMultipleMetrics
        ...
        # pipeline.
        echo Calling ConvertSequencingArtifactToOxoG
    ...
    output {
    	# TODO change outputs on the workflow level (converted_oxog_metrics)       
...
task Mutect1_Task {
    ...
    Int diskGB = ceil(tumorBam_size + normalBam_size + refFasta_size + db_snp_vcf_size
                ...
                + size(normalPanel, "G") + select_first([diskGB_buffer, default_diskGB_buffer]))
...
task Mutect2_Task {
    ...
    Int diskGB = ceil(tumorBam_size + normalBam_size + refFasta_size + gatk4_jar_size
                ...
                + size(normalPanel, "G") + machine_diskGB_buffer)
...
task MutectFC_Task {
    ...
    Int diskGB = ceil(tumorBam_size + normalBam_size + refFasta_size + db_snp_vcf_size
                ...
                + size(readGroupBlackList, "G") + size(normalPanel, "G") + machine_diskGB_buffer)
...
task DeTiN_Task {
    ...
    String default_release_version = "v1.8.4"
    ...
    command <<<
        ...
        echo null > ${pairName}.TiN_estimate.txt
        echo null > ${pairName}.TiN_estimate_CI.txt
        echo null > ${pairName}.deTiN_aSCNAs.txt
        echo null > ${pairName}.deTiN_SSNVs.txt
        echo null > ${pairName}.deTiN_indels.txt
        echo null > ${pairName}_SSNVs_plot.png
        echo null > ${pairName}_KmeansEval_plot.png
        echo null > ${pairName}_TiN_models_plot.png
        echo null > ${pairName}_KmeansEval_scatter_plot.png
...
task Oncotate_Task {
    ...
    runtime {
        docker: "mcadosch/cclf_wes:latest"
        bootDiskSizeGb: if diskGB_boot != "" then diskGB_boot else default_diskGB_boot
        preemptible: if preemptible != "" then preemptible else default_preemptible
        cpu: if cpu != "" then cpu else default_cpu
        disks: "local-disk ${diskGB} HDD"
...
task OrientationBias_filter_Task {
    ...
    output {      
        ...
        File WXS_Mutation_OBF_filtered_maf="${caseName}.OrientationBiasFilter.${stub}.indel_snp_merged.filtered.maf"
        File WXS_Mutation_OBF_unfiltered_maf="${caseName}.OrientationBiasFilter.${stub}.indel_snp_merged.unfiltered.maf"
    }
}
...
task MAFPonFilter {
    ...
    String default_public_release = "true"
    ...
    command <<<
        ...
        then
            python /usr/local/bin/remove_columns.py --IN_MAF "${pairName}.pon_annotated.pass.maf" --OUT_MAF "${pairName}.pon_annotated.pass.public.maf"
            python /usr/local/bin/remove_columns.py --IN_MAF "${pairName}.pon_annotated.maf" --OUT_MAF "${pairName}.pon_annotated.public.maf"
    ...
    output {
        ...
        File allMaf="${pairName}.pon_annotated.maf"
        File passMaf="${pairName}.pon_annotated.pass.maf"
    }
}
...
task merge_mafs_task {
    ...
    File oxoGOBF_maf
    File ffpeOBF_maf
    File agilent_pon_maf
    File ice_pon_maf
    File blat_maf
    #File VEP_annotated_maf
    String pairName
    ...
    # COMPUTE DISK SIZE
    Int machine_diskGB_buffer = if diskGB_buffer != "" then diskGB_buffer else default_diskGB_buffer
    Int diskGB = ceil(size(oxoGOBF_maf, "G") + size(ffpeOBF_maf, "G") + size(agilent_pon_maf, "G") +size(ice_pon_maf, "G") +size(blat_maf, "G") 
                     + machine_diskGB_buffer)

    parameter_meta {
        oxoGOBF_maf     : "MAF with mutations that passsed OxoG Orientation Bias Filter"
        ffpeOBF_maf     : "MAF with mutations that passed FFPE Orientation Bias Filter"
        agilent_pon_maf : "MAf with mutations that passed Agiletn PoN MAFPoN Filter"
        ice_pon_maf     : "MAF"
        blat_maf        : ""
    }
    ...
    command <<<
        ...
        MERGED_INTERSECTION_MAF="${pairName}.filters_passed_maf.intersection.maf"
        python /usr/local/bin/merge_filter_passed_mafs.py \
        --oxogOBF=${oxoGOBF_maf} \
        --ffpeOBF=${ffpeOBF_maf} \
        --agilent_pon=${agilent_pon_maf} \
        --ice_pon=${ice_pon_maf} \
        --blat=${blat_maf} \
        --method='INTERSECTION' \
        --output=$MERGED_INTERSECTION_MAF

        MERGED_UNION_MAF="${pairName}.filters_passed_maf.union.maf"
        python /usr/local/bin/merge_filter_passed_mafs.py \
        --oxogOBF=${oxoGOBF_maf} \
        --ffpeOBF=${ffpeOBF_maf} \
        --agilent_pon=${agilent_pon_maf} \
    ...
    output {
        File merged_intersection_maf="${pairName}.filters_passed_maf.intersection.maf"
        File merged_union_maf="${pairName}.filters_passed_maf.union.maf"
    }
}
...
task gatk_acnv_only{
    ...
    command <<<
        ...
        /usr/local/jre1.8.0_73/bin/java "-Xmx${command_memoryGB}g" -Djava.library.path=/usr/lib/jni/ \
        -jar ${gatk_protected} CallSegments \
        --tangentNormalized ${pairName}.tn.tsv \
        --segments ${pairName}.seg \
        --output ${pairName}.called \
        --legacy false \
        --help false \
        --version false \
        --verbosity INFO \
...
task absolute {
    ...
    command <<<
        ...
        convert ${pairName}.ABSOLUTE_plot.pdf ${pairName}.compressed.pdf
    ...
    output {
        File absolute_plot="${pairName}.compressed.pdf"
